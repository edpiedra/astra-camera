...
class Viewer3D():
    ...
    def _extract_hsv_inrange(self):
        ...
        cv2.putText(
            result, self.hsv_min, (5,10), cv2.FONT_HERSHEY_SIMPLEX, 
            0.5, (0,0,255), 2
        )
        
        cv2.putText(
            result, self.hsv_max, (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 
            0.5, (0,0,255), 2
        )
    ...
    def _find_circle(self):
        ...
        if len(cnts) > 0:
            ...
            if radius > 10:
                ...
                cv2.circle(self.color_frame, 
                           ball_center,
                           int(radius),
                           (0,255,0),
                           2)
                
                roi_margin = int(radius * .20) #can be adjusted to reflect the portion of the ball to test
